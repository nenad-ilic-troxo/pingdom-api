...
class PingdomApi
{
    ...
    public function ensureParameters($parameters = array(), $method)
    {
        ...
        foreach ($parameters as $parameter => $value) {
            if (!isset($value)) {
                throw new MissingParameterException(sprintf('Missing required %s parameter in %s', $parameter,
                  $method));
            }
        }
    }
    ...
    public function addCheck($check, $defaults = array())
    {
        ...
        return sprintf('Created check %s for %s at http://%s%s', $data->check->id, $check['name'], $check['host'],
          $check['type']);
    }
